...
const Swap = () => {
  ...
  const priceBias =
    (animatedPrice - selectedCoin.priceNgn) / selectedCoin.priceNgn;
  ...
  return (
    <PageLayout description="A self-serve swap page for AyoCoin." title="Swap">
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-2 px-3 md:gap-3 md:px-0">
        {/* Coin Header Card */}
        <Card
          className="overflow-hidden border-0 bg-[#0b0b0c] p-2.5 text-white md:border md:border-gray-200 md:bg-white md:p-4 md:text-gray-900 dark:md:border-gray-700 dark:md:bg-black dark:md:text-gray-100"
          forceRounded
        >
          <div className="flex items-center justify-between gap-1.5 md:gap-3">
            ...
            <div className="flex flex-shrink-0 items-center gap-1.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400 md:text-gray-500 md:text-xs">
                  {selectedCoin.handle}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-shrink-0 items-center gap-2">
              <div className="text-right">
                <p className="font-bold text-base text-white md:text-gray-900 md:text-xl dark:md:text-gray-100">
                  ₦{Math.round(animatedPrice).toLocaleString()}
                </p>
                <div className="mt-1 flex items-center gap-1 font-semibold text-[11px] text-gray-300 md:text-gray-600 md:text-xs">
                  ...
                  >
                    {trendUp ? "▲" : "▼"}{" "}
                    {Math.abs(selectedCoin.percentChange).toFixed(1)}
                  </span>
                </div>
              </div>
              <button
        ...
        <Card
          className="overflow-hidden border-0 bg-[#111217] p-3 text-white shadow-none md:p-4"
          forceRounded
        >
          <div className="relative space-y-3">
            <div className="rounded-2xl border-0 bg-[#1a1c22] p-3 shadow-none ring-0 md:p-3.5">
              <div className="flex items-center justify-between font-semibold text-[#8b8b95] text-[11px] uppercase tracking-[0.2em]">
                <span>From</span>
                <span>
                  Balance:{" "}
                  {fromIsFiat
              ...
              <div className="mt-2 flex items-center gap-3">
                <input
                  ...
                  className="w-full bg-transparent font-semibold text-2xl text-white outline-none placeholder:text-[#6b6b73]"
                  onChange={(event) => {
                ...
                />
                {fromIsFiat ? (
                  <div className="inline-flex items-center gap-2 rounded-full bg-[#242733] px-3 py-2 font-semibold text-sm text-white">
                    <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#0c0d12] font-semibold text-[11px]">
                      ₦
                    </span>
                    NGN
                  </div>
                  ...
                  <button
                    className="inline-flex items-center gap-2 rounded-full bg-[#242733] px-3 py-2 font-semibold text-sm text-white"
            ...
            <button
              className="absolute top-[50%] left-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#2c303b] bg-[#111217] p-2 text-gray-200 shadow-sm"
              onClick={() =>
                setDirection((prev) =>
          ...
          <div className="mt-3 flex items-center justify-between text-[#9a9aa2] text-[11px]">
            <span>
              {fromIsFiat
            ...
            <span>
              Rate: ₦{selectedCoin.priceNgn.toLocaleString()} /{" "}
              {selectedCoin.symbol}
            </span>
          </div>

          {/* Primary CTA Button */}
          <Button className="mt-3 w-full rounded-2xl border-none bg-green-600 py-3 font-semibold text-base text-white hover:bg-green-700">
            Buy {selectedCoin.name}
          </Button>
          <p className="mt-1 text-center text-[#8e8e95] text-xs">
        ...
        <Card className="overflow-hidden p-3 md:p-3.5" forceRounded>
          <div className="mb-1 flex items-center justify-between md:mb-2">
            <p className="font-semibold text-gray-900 text-xs md:text-sm dark:text-gray-100">
              Select coin
            </p>
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] md:gap-2 [&::-webkit-scrollbar]:hidden">
            {coins.map((entry) => {
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[#9a9aa2] text-xs">₦{coin.symbol}</p>
                      </div>
                    </div>
                    <div className="text-right">

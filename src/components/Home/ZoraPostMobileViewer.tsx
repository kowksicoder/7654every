...
import { Fragment, memo, useEffect, useRef, useState, type UIEvent } from "react";
...
const ZoraPostMobileViewer = ({
  ...
  const [commentDrawerItem, setCommentDrawerItem] = useState<ZoraFeedItem | null>(null);
  const [profileDrawerItem, setProfileDrawerItem] = useState<ZoraFeedItem | null>(null);
  ...
  const handleScroll = (event: UIEvent<HTMLDivElement>) => {
    ...
    if (remainingDistance < node.clientHeight * 1.25 && hasNextPage && !isFetchingMore) {
      onRequestMore();
    }
  };
      ...
      >
        ...
        <div className="fixed inset-0 md:hidden">
          <DialogPanel className="relative h-full w-full overflow-hidden bg-black text-white">
            <button
              className="absolute top-[max(env(safe-area-inset-top),1rem)] left-4 z-30 inline-flex size-10 items-center justify-center rounded-full text-white"
              onClick={onClose}
              type="button"
            >
            ...
            >
              {items.map((item, index) => {
                ...
                const videoSource =
                  item.mediaContent?.videoHlsUrl || item.mediaContent?.videoPreviewUrl;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute top-0 right-0 left-0 z-10 flex justify-center px-4 pt-[max(env(safe-area-inset-top),1rem)]">
                      <div className="flex items-center gap-2 rounded-full px-2 py-2">
                        ...
                        <button
                          className="inline-flex items-center rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-gray-950"
                          type="button"
                        >
                          Trade
                        </button>
                    ...
                    <div className="absolute right-0 bottom-0 left-0 z-10 px-4 pb-[calc(env(safe-area-inset-bottom)+1.5rem)]">
                      <div className="relative min-h-[15rem]">
                        <div className="absolute right-0 bottom-0 left-0 pr-20">
                          ...
                          <p className="mt-1 text-[13px] text-white/80">{creatorName}</p>

                          <p className="mt-3 max-w-full text-[14px] leading-6 text-white/90 [display:-webkit-box] overflow-hidden break-words [-webkit-box-orient:vertical] [-webkit-line-clamp:4] [overflow-wrap:anywhere]">
                            {caption ? truncateByWords(caption, 34) : item.name}
                          </p>

                          <div className="mt-4 flex flex-wrap gap-2">
                            <span className="inline-flex items-center rounded-full bg-black/30 px-3 py-1 text-[11px] font-semibold text-white/90 ring-1 ring-white/15 backdrop-blur-sm">
                        ...
                        <div className="absolute top-0 right-2 flex w-14 shrink-0 -translate-y-2 flex-col items-center gap-3.5">
                          ...
                          <ViewerAction count={likeCount} icon={HeartIcon} label="Like" />
                          <ViewerAction
                            count={commentCount}

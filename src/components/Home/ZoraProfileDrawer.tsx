...
import { getProfile, type GetProfileResponse } from "@zoralabs/coins-sdk";
...
const ZoraProfileDrawer = ({
  item,
  onClose,
  show
}: ZoraProfileDrawerProps) => {
  ...
  const marketDelta = Number.parseFloat(profile?.creatorCoin?.marketCapDelta24h ?? "0");
  const isPositive = marketDelta >= 0;
  const walletAddress = profile?.publicWallet?.walletAddress || item?.creatorAddress || item?.address;
  const socialAccountCount = Object.values(profile?.socialAccounts ?? {}).filter(Boolean).length;
          ...
          <div className="absolute inset-x-0 bottom-0 max-h-[72vh] overflow-hidden rounded-t-[1.75rem] bg-white text-gray-950 shadow-2xl dark:bg-gray-950 dark:text-gray-50">
            ...
            {isLoading ? (
              ...
              <div className="px-5 pt-4 pb-[max(env(safe-area-inset-bottom),1rem)]">
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      {socialAccountCount ? (
                        <span className="inline-flex items-center rounded-full bg-gray-100 px-3 py-1 text-[11px] font-semibold text-gray-700 dark:bg-gray-900 dark:text-gray-200">
                ...
                {bio ? (
                  <p className="mt-4 text-[13px] leading-6 text-gray-700 break-words [overflow-wrap:anywhere] dark:text-gray-300">
                    {bio}
                  </p>
                ) : null}

                {website ? (
                  <p className="mt-3 truncate text-[13px] font-medium text-blue-600 dark:text-blue-400">
                    {website}
                  </p>
                ) : null}
                ...
                <div className="mt-5 grid grid-cols-3 gap-2">
                  ...
                  <div className="rounded-2xl bg-gray-100 px-3 py-3 text-center dark:bg-gray-900">
                    <p className="text-sm font-semibold">{formatUsdMetric(marketCap)}</p>
                    <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                      Coin MC
                    </p>
                  </div>
                ...
                <div className="mt-5 flex items-center gap-3">
                  <button
                    className="inline-flex flex-1 items-center justify-center rounded-full bg-gray-950 px-4 py-3 text-sm font-semibold text-white dark:bg-white dark:text-gray-950"
                    type="button"
                  >
                    Trade
                  </button>
                  <button
                    className="inline-flex flex-1 items-center justify-center rounded-full bg-gray-100 px-4 py-3 text-sm font-semibold text-gray-700 dark:bg-gray-900 dark:text-gray-200"
                    onClick={onClose}
                    type="button"
                  >
                    Close
